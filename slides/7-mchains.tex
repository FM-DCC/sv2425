...
\item $\fdec{Inv}{L}{\cc{C}\cup rate}$ is the assignment of \structure{invariants} \alert{or rates (of an exponential distribution)} to locations  
\end{itemize}
\vspace*{-1mm}
...
\begin{frame}[t]\frametitle{Again A1,A2,A3: Timed PTS}
  ...
  \splittwo{0.4}{0.58}{
    ...
    \begin{itemize}
      \item Probability of $\pair{A1_0,0} \trans{0.5} \pair{A1_0,0.5}$?
      % \pause
      \item Probability of $\pair{A2_0,0} \trans{0.5} \pair{A2_0,0.5}$?
      \pause
      \item Probability of $\pair{A3_0,0} \trans{0.5} \pair{A3_0,0.5}$?
      \pause
...
\begin{frame}[t]\frametitle{Generative Timed PTS}
  \centering
  {\large \alert{$\alpha: S \to \Dist((S\times N) + 1)$}}\\
  {\Large \structure{$\alpha: S \to \Dist(S\times (Act+R+1))$}}


  \bigskip

  \splittwo{0.46}{0.52}{
  \begin{alertblock}{Before (PTS)}\centering
    \wrap{\begin{tikzpicture}[aut]
    \node[st,initial above,final]  (s_0) {$A_0$};
    \node[st] (s_1) [above right of=s_0] {$A_1$};
    \node[st] (s_2) [below right of=s_0] {$A_2$};
  ...
  \doExercise{Now (Timed PTS) -- formalise it}{\centering
    ...
    \path[->]
      ...
      (s_0) edge  node {\alert{b[0.2]}}  (s_2)
      (s_1) edge  node {\structure{10[0.3]}}  (s_2)
      (s_0) edge[loop left] node {\alert{a[0.4]}}  ()
      (s_1) edge[loop right] node {\structure{b[0.3]}}  (s_1);
      % (s_2) edge[loop right] node {1}  (s_1);
  \end{tikzpicture}}
